...
export default {
  ...
  computed: {
    previewImage() {
      return !this.autoCrop && !this.croppedImage
        ? this.image
        : this.croppedImage;
    },
    cropSizes() {
      ...
      const aspectRatio =
        this.cropOptions.aspectRatio || originalWidth / originalHeight;
  ...
  methods: {
    smartCrop() {
      if (this.autoCrop) {
        ...
        img.src = this.image;
        img.onload = () => {
          smartcrop.crop(img, this.cropSizes).then(this.onSmartCropped);
        };
      }
    },
    cropperReady() {
